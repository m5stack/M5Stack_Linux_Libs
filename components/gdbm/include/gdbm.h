...
# define GDBM_BSEXACT   0x0200  /* Don't adjust block_size. Bail out with GDBM_BLOCK_SIZE_ERROR error if unable to set it. */  
...
# define GDBM_GETDIRDEPTH     18 /* Directory depth: number of initial (mostsignificant) bits in hash interpreted asindex to the directory. */
...
enum gdbm_latest_snapshot_status
  {
    ...
    GDBM_SNAPSHOT_SUSPICIOUS /* Selected snapshot is unreliable: numsyncs differ by more than 1. */
  };
...
#define GDBM_RCVR_BACKUP               0x10  /* Keep backup copy of theoriginal data base on success */
#define GDBM_RCVR_FORCE                0x20  /* Force recovery by skipping the check pass */
