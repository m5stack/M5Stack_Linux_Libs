...
typedef enum { ENCODE_STATE_STOP, ENCODE_STATE_START } eEncodeState;
...
static int H264FrameEncodeBegin(uint32_t Width, uint32_t Height) {
    ...
    x264_picture_alloc(sEncodePrivateData.pPicIn, CONFIG_CSP,
                       sEncodePrivateData.pParam->i_width,
                       sEncodePrivateData.pParam->i_height);
...
static int H264FrameEncode(const uint8_t* pData, uint32_t Width,
    ...
    if (x264_encoder_encode(sEncodePrivateData.pHandle,
                            &(sEncodePrivateData.pNals), &NalCnt,
                            sEncodePrivateData.pPicIn, pPicOut) < 0) {
        free(pPicOut);
        return -1;
    }
    for (int i = 0; i < NalCnt; i++) {
        // memcpy(pOut, sEncodePrivateData.pNals[i].p_payload,
        // sEncodePrivateData.pNals[i].i_payload); pOut +=
        // sEncodePrivateData.pNals[i].i_payload;
        fwrite(sEncodePrivateData.pNals[i].p_payload, 1,
               sEncodePrivateData.pNals[i].i_payload, sEncodePrivateData.pFile);
...
static void* H264EncodeThread(void* pParam) {
    ...
    while (sEncodePrivateData.State == ENCODE_STATE_START) {
        ...
        H264FrameEncode(QueueData.pData, CONFIG_CAPTURE_WIDTH,
                        CONFIG_CAPTURE_HEIGHT, QueueData.Length);
...
int EncodeStart(const char* pFilename) {
    ...
    pthread_create(&sEncodePrivateData.EncodeThreadId, NULL, H264EncodeThread,
                   (void*)NULL);
