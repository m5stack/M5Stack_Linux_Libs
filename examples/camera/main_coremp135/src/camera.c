...
static struct {
    ...
    eCAMERA_STATE State; /* 0 Not open, 1 Turn on the camera, 2 Start capturing,
                            3 Error */
...
//  } while(Ret && Tries-- && ((errno == EINTR) || (errno == EAGAIN) || (errno
//  == ETIMEDOUT)));

//  if(Ret && (Tries <= 0)) fprintf(stderr, "ioctl (%i) retried %i times -
...
int Yuyv2Rgb32(uint8_t* pYuv, uint8_t* pRgb, uint32_t Width, uint32_t Height) {
    ...
    // characters, so to calculate the total number of characters here, you need
    // to multiply by 2.
    ...
    for (uint32_t In = 0, Out = 0; In < Size; In += 4, Out += 8) {
        ...
        pRgb[Out + 1] = pPixel[1];
        pRgb[Out + 2] = pPixel[2];
        pRgb[Out + 3] = 0xff;  // 32-bit RGB has an extra reserved bit.
...
int Yuyv2Rgb24(uint8_t* pYuv, uint8_t* pRgb, uint32_t Width, uint32_t Height) {
    ...
    uint8_t* pPixel = (uint8_t*)&Pixel32;
    // Resolution describes the number of pixels, and each YUV pixel occupies 4
    // characters, so the total number of characters here needs to be multiplied
    // by 2.
    uint32_t Size = Width * Height * 2;
...
int Yuyv2Rgb16(uint8_t* pYuv, uint8_t* pRgb, uint32_t Width, uint32_t Height) {
    int y0, U, y1, V;
    uint32_t Pixel32;
    uint8_t* pPixel = (uint8_t*)&Pixel32;
    // Resolution describes the number of pixels, and each YUV pixel occupies 4
    ...
    for (uint32_t In = 0, Out = 0; In < Size; In += 4, Out += 2) {
        y0 = pYuv[In + 0];
        U  = pYuv[In + 1];
        y1 = pYuv[In + 2];
        V  = pYuv[In + 3];
        ...
        Sign3           = 1;
        Pixel32         = Yuv2Rgb(y0, U, V);
        pRgb16[Out + 0] = rgb24_to_rgb565(pPixel[2], pPixel[1], pPixel[0]);

        Pixel32         = Yuv2Rgb(y1, U, V);
        pRgb16[Out + 1] = rgb24_to_rgb565(pPixel[2], pPixel[1], pPixel[0]);
    }
    return 0;
}
...
int CameraOpen(const char* pDevName) {
    ...
    if (sCameraPrivateData.State != CAMERA_STATE_CLOSE) {
        printf("Error: camera was open or meet error, now state is: %d",
               sCameraPrivateData.State);
        goto ErrorHandle;
    }

    /* Open the camera device */
    if ((sCameraPrivateData.DevFd = open(pDevName, O_RDWR, 0)) == -1) {
        // if((sCameraPrivateData.DevFd = open(pDevName, O_RDWR | O_NONBLOCK,
        // 0)) == -1) {
    ...
    sCameraPrivateData.pCameraBuffer =
        (sBufferType*)calloc(sCameraPrivateData.BufferCnt, sizeof(sBufferType));
    ...
    for (int i = 0; i < sCameraPrivateData.BufferCnt; i++) {
        ...
        sCameraPrivateData.pCameraBuffer[i].pStart =
            mmap(NULL, Buf.length, PROT_READ | PROT_WRITE, MAP_SHARED,
                 sCameraPrivateData.DevFd, Buf.m.offset);
...
static void* CameraCaptureThread(void* pParam) {
    ...
    while (sCameraPrivateData.State == CAMERA_SATTE_CAP) {
        ...
        sCameraPrivateData.pCallback(
            sCameraPrivateData.pCameraBuffer[DeQueueBuf.index].pStart,
            sCameraPrivateData.Width, sCameraPrivateData.Height,
...
int CameraCaptureStart(void) {
    ...
    pthread_create(&sCameraPrivateData.CaptureThreadId, NULL,
                   CameraCaptureThread, (void*)NULL);
