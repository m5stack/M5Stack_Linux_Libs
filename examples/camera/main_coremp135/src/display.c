...
typedef enum { DISPLAY_STATE_STOP, DISPLAY_STATE_START } eDisplayState;
...
static const int BufferSize = CONFIG_CAPTURE_WIDTH * CONFIG_CAPTURE_HEIGHT * 2;

extern int Yuyv2Rgb32(uint8_t* pYuv, uint8_t* pRgb, uint32_t Width,
                      uint32_t Height);
extern int Yuyv2Rgb16(uint8_t* pYuv, uint8_t* pRgb, uint32_t Width,
                      uint32_t Height);
int DisplayInit(const char* pDevName) {
    ...
    sDisplayPrivateData.ScreenSize =
        FbVar.xres * FbVar.yres * (FbVar.bits_per_pixel / 8);

    printf("Screen size: %d\n", sDisplayPrivateData.ScreenSize);

    sDisplayPrivateData.pMmap =
        mmap(NULL, sDisplayPrivateData.ScreenSize, PROT_READ | PROT_WRITE,
             MAP_SHARED, sDisplayPrivateData.DevFd, 0);
...
static void* DisplayThread(void* pParam) {
    ...
    while (sDisplayPrivateData.State == DISPLAY_STATE_START) {
        ...
        // Yuyv2Rgb32(QueueData.pData, pRgbBuffer, CONFIG_CAPTURE_WIDTH,
        // CONFIG_CAPTURE_HEIGHT);
        Yuyv2Rgb16(QueueData.pData, pRgbBuffer, CONFIG_CAPTURE_WIDTH,
                   CONFIG_CAPTURE_HEIGHT);
        // printf("Copy length: %d\n", BufferSize);

        // memcpy(sDisplayPrivateData.pMmap, pRgbBuffer, BufferSize);

        uint32_t CopyWidthLength =
            sDisplayPrivateData.ScreenWidth > CONFIG_CAPTURE_WIDTH
                ? CONFIG_CAPTURE_WIDTH
                : sDisplayPrivateData.ScreenWidth;

        for (int i = 0; i < CONFIG_CAPTURE_HEIGHT; i++) {
            memcpy(sDisplayPrivateData.pMmap +
                       sDisplayPrivateData.ScreenWidth * 2 * i,
                   &pRgbBuffer[CONFIG_CAPTURE_WIDTH * 2 * i],
                   CopyWidthLength * 2);
        }
...
int DisplayStart(void) {
    ...
    pthread_create(&sDisplayPrivateData.DisplayThreadId, NULL, DisplayThread,
                   (void*)NULL);
    return 0;
}
