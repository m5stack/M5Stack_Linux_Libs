...
int main(int argc, char** argv)
{
    ...
    for (int i = 0; i < clientNum; i++) {
        ...
        auto enterCallback = [packetLen](const TcpConnection::Ptr& dataSocket) {
            ...
            dataSocket->setDisConnectCallback([](const TcpConnection::Ptr& dataSocket) { (void)dataSocket; });
        };
        auto tcpConnection = TcpConnection::Create(TcpSocket::Create(fd, false), 1024 * 1024, enterCallback,
                                                   clientEventLoop, nullptr, nullptr);
    }
