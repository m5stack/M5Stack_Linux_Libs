...
extern int mqtt_Creat_Msg(char **Msg, const char *dev_eui, int confirmed, int fPort, void *data, int data_len);
...
void lora_enquene_msg_cb(lv_timer_t *tmr) {
    ...
    if ((mqtt_cli != NULL) && mqtt_client_is_connected(mqtt_cli)) {
        ...
        msg.payload_len = mqtt_Creat_Msg(&msg.payload, "4bee530d29435bef", false, 10, &val, sizeof(val));
        val++;
        ...
        DownLink_cout++;
        int index = next_display_count();
        sprintf(strbuff, "%d", 10);
        lv_label_set_text(positiono[index][0], strbuff);
        sprintf(strbuff, "%s DevEUI:%s", get_times(), "4bee530d29435bef");
        lv_label_set_text(positiono[index][1], strbuff);
        sprintf(strbuff, "Freq:%s BW:%s SF:%s Rssi:%s", "868.0M", "125K", "7", "-60");
        lv_label_set_text(positiono[index][2], strbuff);
        format_int(val, strbuff);
        lv_label_set_text(positiono[index][3], strbuff);
        lv_label_set_text(positiono[index][4], "down");
        lv_img_set_src(positionoB[index], &ui_img_rectangle_15_png);
        lv_obj_clear_flag(positionoA[index], LV_OBJ_FLAG_HIDDEN);
        sprintf(strbuff, "%d", DownLink_cout);
        lv_label_set_text(ui_Label7, strbuff);
    }
}