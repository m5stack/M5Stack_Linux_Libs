...
AX_S32 SampleMemInit(SAMPLE_VENC_CMD_PARA_T *pCml)
{
    ...
    if (AX_SUCCESS != s32Ret) {
        SAMPLE_LOG_ERR("AXCL_SYS_Init failed! Error Code:0x%X\n", s32Ret);
        return -1;
    }
    ...
    pCml->poolId = AXCL_POOL_CreatePool(&stPoolConfig);
...
FREE_SYS:
    s32Ret = AXCL_SYS_Deinit();
    if (AX_SUCCESS != s32Ret)
        SAMPLE_LOG_ERR("AXCL_SYS_Deinit failed! Error Code:0x%X\n", s32Ret);
...
AX_S32 SampleMemDeinit(SAMPLE_VENC_CMD_PARA_T *pCml)
{
    ...
    if (AX_INVALID_POOLID != pCml->poolId) {
        s32Ret = AXCL_POOL_DestroyPool(pCml->poolId);
        if (s32Ret) {
    ...
    if (pCml->bDynRes && (AX_INVALID_POOLID != pCml->poolIdDynRes)) {
        s32Ret = AXCL_POOL_DestroyPool(pCml->poolIdDynRes);
        if (s32Ret) {
