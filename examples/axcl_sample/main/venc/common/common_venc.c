                         ...
                         SAMPLE_VENC_CMD_PARA_T *pstArg)
{
    ...
    s32Ret = AXCL_VENC_StartRecvFrame(VeChn, &stRecvParam);
    if (AX_SUCCESS != s32Ret) {
        SAMPLE_LOG_ERR("chn-%d: AXCL_VENC_StartRecvFrame failed with%#x! \n", VeChn, s32Ret);
        return -1;
    }
...
AX_S32 COMMON_VENC_Stop(VENC_CHN VeChn)
{
    ...
    s32Ret = AXCL_VENC_StopRecvFrame(VeChn);
    if (AX_SUCCESS != s32Ret) {
        SAMPLE_LOG_ERR("chn-%d: AX_VENC_StopRecvFrame failed with%#x! \n", VeChn, s32Ret);
        return -1;
    }

    s32Ret = AXCL_VENC_DestroyChn(VeChn);
    if (AX_SUCCESS != s32Ret) {
        SAMPLE_LOG_ERR("chn-%d: AXCL_VENC_DestroyChn failed with%#x! \n", VeChn, s32Ret);
        return -1;
    }
                          ...
                          SAMPLE_VENC_CMD_PARA_T *pstArg)
{
    ...
    /* create channel */
    s32Ret = AXCL_VENC_CreateChn(VeChn, &stVencChnAttr);
    if (AX_SUCCESS != s32Ret) {
        SAMPLE_LOG_ERR("chn-%d: AXCL_VENC_CreateChn failed, ret=0x%x\n", VeChn, s32Ret);
        goto END;
    }
...
AX_VOID *COMMON_VENC_SendFrameProc(AX_VOID *arg)
{
    ...
    if (fFileIn == NULL) {
        SAMPLE_LOG_ERR("chn-%d: Open input file error! %d %s\n", VeChn, errno, strerror(errno));
        return NULL;
    }
    ...
    while (pstArg->bSendFrmStart) {
        ...
        if (AX_SUCCESS != s32Ret)
            SAMPLE_LOG_WARN("chn-%d: AXCL_VENC_SendFrame failed, ret=%x\n", VeChn, s32Ret);
...
AX_VOID *COMMON_VENC_GetStreamProc(AX_VOID *arg)
{
    ...
    if (NULL == pStrm) {
        if (NULL == pstArg->output) {
            SAMPLE_LOG_ERR("chn-%d: Open output file %s error! %d %s\n", VeChn, esName, errno, strerror(errno));
        } else {
            SAMPLE_LOG_ERR("chn-%d: Open output file %s error! %d %s\n", VeChn, pstArg->output, errno, strerror(errno));
        }
        return NULL;
    }
                                ...
                                AX_BOOL bSaveFile)
{
    ...
    if (NULL == pStrm) {
        SAMPLE_LOG_ERR("chn-%d: Open output file %s error! %d %s\n", VeChn, fileName, errno, strerror(errno));
        return -1;
    }
